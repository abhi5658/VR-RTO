...
public class SimpleCarController : MonoBehaviour
{
	...
	public float maxAccelerationTorque = 350f;
	...
	private float turn = 0.0f, acc = 0.0f, brake = 0.0f;
	string gamedata="";
    ...
    public void Start()
    {
		...
		socket.On("datarec",(SocketIOEvent obj)=> {
			...
			var Hval = N["R"].AsFloat;
			var Vval = N["A"].AsFloat;
			var Bval = N["B"].AsFloat;

			//Debug.Log("------jsoning---{ H : "+Hval+" V : "+ Vval + " }");

			acc = Vval;//acceleration
			turn = Hval;//turn
			brake = Bval;//brake
    ...
    {
		float steer, accelerationTorque, brakeTorque;
		/*
		if (socketdata == true) 
		{*/
			steer = turn * maxSteerAngle;
			accelerationTorque = acc * maxAccelerationTorque;
...
*/
        ...
        for (int i = 2; i < 4; i++) {
			wheelCollider[i].motorTorque = accelerationTorque;
